...
sub add_or_update_package {
    ...
    @output = qx( aptly publish repo -distribution=$version -component=$shortname $version-$shortname ) if $is_new;
    if ( $verbose > 3 && $is_new ) { say for ( "Publishing $version-$shortname: ", @output ); }
    @output = qx( aptly publish update $version );
    if ( $verbose > 3 ) { say for ( "Updating $version: ", @output ); }
}
...
The distribution will be $version
The component will be $shortname
...
sub create_repo {
	...
	@output = qx( aptly repo create -distribution=$version -component=$shortname $version-$shortname );
